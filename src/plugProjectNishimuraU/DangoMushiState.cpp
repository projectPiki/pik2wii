...
void FSM::init(EnemyBase* enemy)
{
	...
	registerState(new StateRecover("recover"));
	...
	if (crab->mCurAnim->mIsPlaying) {
		if (crab->mCurAnim->mType == KEYEVENT_END_BLEND) {
			...
			if (target && gameSystem && gameSystem->mMode != GSM_PIKLOPEDIA) {
				f32 minAttackRange = CG_GENERALPARMS(crab).mMaxAttackAngle();
				f32 maxAttackRange = CG_GENERALPARMS(crab).mMaxAttackRange();
