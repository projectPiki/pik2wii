...
void IKSystemMgr::setNextCentrePosition()
{
	...
	Vector3f nextPos;
	if (isAngleWithin(angleDist, mParams->mEnragedAngle)) {
		...
		nextPos.y = 0.0f;
		if (!(isAngleWithin(angleDist, mParams->mMaxTurnAngle))) {
			angleDist = (angleDist > 0.0f) ? TORADIANS(mParams->mMaxTurnAngle) : -TORADIANS(mParams->mMaxTurnAngle);
		}
	}
