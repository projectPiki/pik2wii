...
// hack floats into correct order
 ...
 *
 */
void AABB::Set(const VEC3* pPoints, uint num)
{
	min = pPoints[0];
	max = pPoints[0];
...
{
	if (pA->min.x > pB->max.x || pB->min.x > pA->max.x || pA->min.y > pB->max.y || pB->min.y > pA->max.y || pA->min.z > pB->max.z
	    || pB->min.z > pA->max.z) {
		return false;
	}
