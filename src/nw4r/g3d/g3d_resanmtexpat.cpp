	...
	for (int i = 0; i < TexPatAnmResult::NUM_OF_ANMS; flags >>= ResAnmTexPatMatData::NUM_OF_FLAGS, i++) {
		...
		if (flags & ResAnmTexPatMatData::FLAG_ANM_CONST) {
			...
			const ResAnmTexPatAnmData* pAnmData
			    = static_cast<const ResAnmTexPatAnmData*>(ut::AddOffsetToPtr(pMatData, pAnmDataImpl->toResAnmTexPatAnmData));
...
void ResAnmTexPat::Release()
{
	...
	for (int i = 0; i < numTexture; i++) {
		pResTexArray[i] = ResTex(nullptr);
	}

	for (int i = 0; i < numPalette; i++) {
		pResPlttArray[i] = ResPltt(nullptr);
	}
}
