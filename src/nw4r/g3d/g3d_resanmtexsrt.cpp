...
inline u32 MakeResult(TexSrt* pSrt, const ResAnmTexSrtTexData* pTexData, f32 frame)
{
	...
	return flags
	     & (ResAnmTexSrtTexData::FLAG_ANM_EXISTS | ResAnmTexSrtTexData::FLAG_SCALE_ONE | ResAnmTexSrtTexData::FLAG_ROT_ZERO
...
void ResAnmTexSrt::GetAnmResult(TexSrtAnmResult* pResult, u32 idx, f32 frame) const
{
	...
	for (index = 0; flags != 0; flags >>= ResAnmTexSrtMatData::NUM_OF_FLAGS, index++) {
		...
		const ResAnmTexSrtTexData* pTexData = reinterpret_cast<const ResAnmTexSrtTexData*>(ut::AddOffsetToPtr(pMatData, *pToTexData++));
