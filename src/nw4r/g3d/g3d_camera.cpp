 ...
 *
 */
Camera::Camera(CameraData* pData)
    : ResCommon(pData)
{
}
...
void Camera::SetPerspective(f32 fovy, f32 aspect, f32 near, f32 far)
{
	...
	r.flags
	    &= ~(CameraData::FLAG_PROJ_FRUSTUM | CameraData::FLAG_PROJ_PERSP | CameraData::FLAG_PROJ_ORTHO | CameraData::FLAG_PROJ_MTX_READY);
...
void Camera::GetProjectionTexMtx(math::MTX34* pMtx) const
{
	if (pMtx != nullptr && IsValid()) {
		...
		if (r.flags & CameraData::FLAG_PROJ_ORTHO) {
			C_MTXLightOrtho(*pMtx, r.projTop, r.projBottom, r.projLeft, r.projRight, r.lightScaleS, -r.lightScaleT, r.lightTransS,
			                r.lightTransT);
		} else if (r.flags & CameraData::FLAG_PROJ_FRUSTUM) {
			C_MTXLightFrustum(*pMtx, r.projTop, r.projBottom, r.projLeft, r.projRight, r.projNear, r.lightScaleS, -r.lightScaleT,
			                  r.lightTransS, r.lightTransT);
		} else /* FLAG_PROJ_PERSP */ {
			C_MTXLightPerspective(*pMtx, r.projFovy, r.projAspect, r.lightScaleS, -r.lightScaleT, r.lightTransS, r.lightTransT);
		}
	}
}
...
void Camera::GXSetViewport() const
{
	...
	if (pObj->field_rendering) {
		::GXSetViewportJitter(r.viewportOrigin.x, r.viewportOrigin.y, r.viewportSize.x, r.viewportSize.y, r.viewportNear, r.viewportFar,
		                      r.flags & CameraData::FLAG_VI_ODD_FIELD ? GX_FIELD_ODD : GX_FIELD_EVEN);
	} else {
		::GXSetViewport(r.viewportOrigin.x, r.viewportOrigin.y, r.viewportSize.x, r.viewportSize.y, r.viewportNear, r.viewportFar);
	}
}
