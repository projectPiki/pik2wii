...
/**
 * @brief Local base class for character animation key frame data.
 *
 * @note Size: 0x4.
 */
...
public:
	CResAnmChrFrmBase(const TData* pPtr)
	    : mPtr(pPtr)
	{
	}

	void operator++(int) { mPtr++; }
	void operator--(int) { mPtr--; }

	TDerived operator+(int n) { return TDerived(mPtr + n); }

protected:
	const TData* mPtr; // _00
};

/**
 * @brief Curried template for specifying sizes of frame data.
 *
 * @note Size: 0x4.
 */
template <typename T>
class CResAnmChrFrm : public CResAnmChrFrmBase<T, CResAnmChrFrm<T> /**/> {
	// _00-_04 = CResAnmChrFrmBase
};

/// Instantiated class for 32-bit frame data (all u8s).
template <>
class CResAnmChrFrm<ResAnmChrFrm32Data> : public CResAnmChrFrmBase<ResAnmChrFrm32Data, CResAnmChrFrm<ResAnmChrFrm32Data> /**/> {
public:
	CResAnmChrFrm(const ResAnmChrFrm32Data* pPtr)
	    : CResAnmChrFrmBase(pPtr)
	{
	}

	u8 GetFrame() const { return mPtr->fvs.frame; }

	f32 GetFrameF32() const { return fastcast::U8_0ToF32(&mPtr->fvs.frame); }
...
/// Instantiated class for 48-bit frame data (all shorts).
template <>
class CResAnmChrFrm<ResAnmChrFrm48Data> : public CResAnmChrFrmBase<ResAnmChrFrm48Data, CResAnmChrFrm<ResAnmChrFrm48Data> /**/> {
public:
	CResAnmChrFrm(const ResAnmChrFrm48Data* pPtr)
	    : CResAnmChrFrmBase(pPtr)
	{
	}

	s16 GetFrame() const { return mPtr->frame; }

	f32 GetFrameF32() const { return fastcast::S10_5ToF32(&mPtr->frame); }
	...
	f32 GetSlope() const { return fastcast::S7_8ToF32(&mPtr->slope); }

	// _00-_04 = CResAnmChrFrmBase
};

/// Instantiated class for 96-bit frame data (all floats).
template <>
class CResAnmChrFrm<ResAnmChrFrm96Data> : public CResAnmChrFrmBase<ResAnmChrFrm96Data, CResAnmChrFrm<ResAnmChrFrm96Data> /**/> {
public:
	CResAnmChrFrm(const ResAnmChrFrm96Data* pPtr)
	    : CResAnmChrFrmBase(pPtr)
	{
	}

	f32 GetFrame() const { return mPtr->frame; }

	f32 GetFrameF32() const { return mPtr->frame; }

	f32 GetValue(const ResAnmChrFVSData* /* pFVSData */) const { return mPtr->value; }

	f32 GetSlope() const { return mPtr->slope; }
...
/**
 * @brief Base namespace-style template for animation traits.
 *
 * @note Size: 0x0.
 */
template <typename T>
...
/// Instantiated class for 32-bit frame-value data.
template <>
...
public:
	...
	static TFrame QuantizeFrame(f32 frame) { return fastcast::F32ToU8_0(frame); }
};

/// Instantiated class for 48-bit frame-value data.
template <>
...
public:
	...
	static TFrame QuantizeFrame(f32 frame) { return fastcast::F32ToS10_5(frame); }
};

/// Instantiated class for 96-bit frame-value data.
template <>
...
public:
	...
	static TFrame QuantizeFrame(f32 frame) { return frame; }
};

/// Instantiated class for 8-bit const value data.
template <>
...
/// Instantiated class for 16-bit const value data.
template <>
...
/// Instantiated class for 32-bit const value data.
template <>
...
f32 CalcAnimationFVS(const ResAnmChrFVSData* pFVSData, f32 frame)
{
	...
	CResAnmChrFrm<typename TTraits::TFrmData> last  = TTraits::GetKeyFrame(pFVSData, pFVSData->numFrameValues - 1);
 ...
 */
template <typename T>
f32 CalcResultFVS(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, f32 frame, bool constant)
{
	if (constant) {
...
inline f32 CalcResult32(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, f32 frame, bool constant)
{
	return CalcResultFVS<ResAnmChrFVS32Data>(pNodeData, pAnmData, frame, constant);
}
inline f32 CalcResult48(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, f32 frame, bool constant)
{
	return CalcResultFVS<ResAnmChrFVS48Data>(pNodeData, pAnmData, frame, constant);
}
inline f32 CalcResult96(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, f32 frame, bool constant)
{
 ...
 */
template <typename T>
f32 CalcResultCV(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, u16 numFrame, f32 frame, bool constant)
{
...
inline f32 CalcResultFrm8(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, u16 numFrame, f32 frame,
                          bool constant)
{
	return CalcResultCV<ResAnmChrCV8Data>(pNodeData, pAnmData, numFrame, frame, constant);
}
inline f32 CalcResultFrm16(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, u16 numFrame, f32 frame,
                           bool constant)
{
	return CalcResultCV<ResAnmChrCV16Data>(pNodeData, pAnmData, numFrame, frame, constant);
}
inline f32 CalcResultFrm32(const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, u16 numFrame, f32 frame,
                           bool constant)
{
 ...
 *
 */
const ResAnmChrNodeData::AnmData* GetAnmScale(math::VEC3* pResult, const ResAnmChrNodeData* pNodeData,
 ...
 *
 */
const ResAnmChrNodeData::AnmData* GetAnmRotation(math::MTX34* pResult, math::VEC3* pRawResult, const ResAnmChrInfoData& rInfoData,
                                                 const ResAnmChrNodeData* pNodeData, const ResAnmChrNodeData::AnmData* pAnmData, f32 frame)
{
	...
	case ResAnmChrNodeData::FLAG_ROT_CV8_FMT: {
		x = CalcResultFrm8(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_X_CONST);
		y = CalcResultFrm8(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_Y_CONST);
		z = CalcResultFrm8(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_Z_CONST);
		break;
	}

	case ResAnmChrNodeData::FLAG_ROT_CV16_FMT: {
		x = CalcResultFrm16(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_X_CONST);
		y = CalcResultFrm16(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_Y_CONST);
		z = CalcResultFrm16(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_Z_CONST);
		break;
	}

	case ResAnmChrNodeData::FLAG_ROT_CV32_FMT: {
		x = CalcResultFrm32(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_X_CONST);
		y = CalcResultFrm32(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_Y_CONST);
		z = CalcResultFrm32(pNodeData, pAnmData++, rInfoData.numFrame, frame, flags & ResAnmChrNodeData::FLAG_ROT_Z_CONST);
		break;
	}
 ...
 *
 */
const ResAnmChrNodeData::AnmData* GetAnmTranslation(math::VEC3* pTrans, const ResAnmChrNodeData* pNodeData,
                                                    const ResAnmChrNodeData::AnmData* pAnmData, f32 frame)
{
...
void GetAnmResult_(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_S(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_R(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_SR(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_T(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_ST(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_RT(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
void GetAnmResult_SRT(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);

typedef void (*GetAnmResultFunc)(ChrAnmResult* pResult, const ResAnmChrInfoData& rInfoData, const ResAnmChrNodeData* pNodeData, f32 frame);
...
void ResAnmChr::GetAnmResult(ChrAnmResult* pResult, u32 idx, f32 frame) const
{
	...
	pResult->flags
	    = flags
	    & (ChrAnmResult::FLAG_ANM_EXISTS | ChrAnmResult::FLAG_MTX_IDENT | ChrAnmResult::FLAG_ROT_TRANS_ZERO | ChrAnmResult::FLAG_SCALE_ONE
	       | ChrAnmResult::FLAG_SCALE_UNIFORM | ChrAnmResult::FLAG_ROT_ZERO | ChrAnmResult::FLAG_TRANS_ZERO | ChrAnmResult::FLAG_PATCH_SCALE
	       | ChrAnmResult::FLAG_PATCH_ROT | ChrAnmResult::FLAG_PATCH_TRANS | ChrAnmResult::FLAG_SSC_APPLY | ChrAnmResult::FLAG_SSC_PARENT
...
{
	bool rotZero = pRotTrans->_00 == 1.0f && pRotTrans->_01 == 0.0f && pRotTrans->_02 == 0.0f && pRotTrans->_10 == 0.0f
	            && pRotTrans->_11 == 1.0f && pRotTrans->_12 == 0.0f && pRotTrans->_20 == 0.0f && pRotTrans->_21 == 0.0f
	            && pRotTrans->_22 == 1.0f;
